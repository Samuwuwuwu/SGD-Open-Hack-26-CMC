  ...
  return (
    <section className="box-selection" aria-labelledby="box-selection-title">
      <div className="drop-screen-heading">
        <div><h2 id="box-selection-title" tabIndex={-1}>Same you.<br />{' '}<span className="highlight-word">Different possibilities.</span></h2></div>
        ...
        <div className="box-options" ref={scrollerRef} onScroll={measureScroll} role="region" aria-label="Box choices">
          {candidates.map((box, index) => (
            <article className={`box-option theme-${box.theme}`} key={box.id}>
              <div className="box-option-top"><span>BOX {String(index + 1).padStart(2, '0')}</span>{openedDrops[box.id] && <span>OPENED</span>}</div>
                ...
                <div className="box-option-details">
                  ...
                  <details className="box-more-clues">
                    ...
                    <dl className="box-clues">
                      {box.itemTypeHints?.length > 0 && <div><dt>Item type hints</dt><dd>{box.itemTypeHints.join(' · ')}</dd></div>}
                      {box.itemTeasers?.length > 0 && <div><dt>A peek inside</dt><dd>{box.itemTeasers.map((teaser) => <span className="box-item-teaser" key={teaser}>{teaser}</span>)}</dd></div>}
                      ...
                      {box.nearbyCount > 0 && <div className="box-match-note"><dt>Match note</dt><dd>{box.nearbyCount} {box.nearbyCount === 1 ? 'find matches' : 'finds match'} your quiz more loosely.</dd></div>}
                    </dl>
                  </details>
                </div>
