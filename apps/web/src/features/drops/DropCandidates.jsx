      ...
      {candidates.length > 0 ? (
        <div className="drop-grid">
          {candidates.map((candidate, index) => (
            <article className="drop-card" key={candidate.sku}>
              ...
              <div className="drop-card-content">
                ...
                <div className="drop-card-footer"><strong>${candidate.surplus_price}</strong><button type="button" onClick={() => onReveal(candidate)}>Reveal <span>↗</span></button></div>
              </div>
            </article>
          ))}
        </div>
